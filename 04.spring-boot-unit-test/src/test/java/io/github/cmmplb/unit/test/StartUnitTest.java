...
// 使用 test profile 的配置文件, 文件名格式为 application-{profile}.yml, 该配置文件中的属性会覆盖 application.yml 中的同名属性
// @ActiveProfiles("test")
// 引用外部配置文件, 这些属性源会覆盖其他配置文件中的属性
// @TestPropertySource(locations = {"classpath:application-ext.yml"})
public class StartUnitTest {
    ...
    public void context() {
        ...
        log.info("logback.env:{}", env.getProperty("logback.env"));
    }

    // 常用注解：
    // - @BeforeEach：在每个单元测试方法执行前都执行一遍
    // - @BeforeAll：在每个单元测试方法执行前执行一遍 ( 只执行一次 ) 需要添加static修饰
    ...
    @BeforeEach
    @DisplayName("初始化方法-beforeEach")
    public void beforeEach() {
        log.info("@BeforeEach 初始化方法, 在每个单元测试方法执行前都执行一遍");
    }

    @BeforeAll
    @DisplayName("初始化方法-beforeAll")
    public static void beforeAll() {
        log.info("@BeforeAll 初始化方法 在每个单元测试方法执行前执行一遍 ( 只执行一次 )");
    }

    @Disabled
    @DisplayName("初始化方法-beforeAll")
    public static void disabled() {
        log.info("当前单元测试置为无效, 即单元测试时跳过该测试");
    }
