打包时命令 DskipTests=true 和 -Dmaven.test.skip=true 区别：

1. mvn clean package -DskipTests=true：跳过测试的执行，但测试代码依旧会被编译。
2. mvn clean package -Dmaven.test.skip=true：跳过测试的执行和编译。
