...
public class ProducerServiceImpl implements ProducerService {
    ...
    public void send2SimpleQueue(String message) {
        rabbitTemplate.convertAndSend(RabbitMqConstant.SIMPLE_QUEUE, message);
    }

    @Override
    public void send2WorkQueue(String message) {
        rabbitTemplate.convertAndSend(RabbitMqConstant.WORK_QUEUE, message);
    }

    @Override
    public void send2FanoutQueue(String message) {
        // 指定交换机-将消息发送到交换机, 由交换机来发消息给消费者
        rabbitTemplate.convertAndSend(RabbitMqConstant.FANOUT_EXCHANGE, "", message); // 需要设置第二个参数,否则第一个交换机的参数就变成routingKey了.
    }
    ...
    public void send2TopicQueue(String routingKey, String message) {
        rabbitTemplate.convertAndSend(RabbitMqConstant.TOPIC_EXCHANGE, routingKey, message);
    }
    ...
    public void send2DeadQueue(String routingKey, String message) {
        ...
        rabbitTemplate.convertAndSend(RabbitMqConstant.COMMON_QUEUE, message, m -> {
            MessageProperties messageProperties = m.getMessageProperties();
            //为每条消息设定过期时间, 10m秒以后消息过期
            messageProperties.setExpiration(RabbitMqConstant.MESSAGE_TTL_TIME + "");
            return m;
        });
    }
}
