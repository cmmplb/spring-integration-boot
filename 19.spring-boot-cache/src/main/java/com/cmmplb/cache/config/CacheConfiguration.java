...
public class CacheConfiguration {
     ...
     *  com.cmmplb.cache.service.UserService-@CacheConfig(cacheNames = "user")：配置了该数据访问对象中返回的内容将存储于名为user的缓存对象中，也可以不使用该注解，直接通过@Cacheable自己配置缓存集的名字来定义；
     * @Cacheable：配置了getById函数的返回值将被加入缓存。同时在查询时，会先从缓存中获取，若不存在才再发起对数据库的访问。该注解主要有下面几个参数：
